...
int main(){
    ...
    double ergodic_rate = ErgodicRate::calculate_rayleigh_positive(distance_typical_BS, laplace_transform_interference,
            noise_power, laplace_transform_reflected_signals);

    auto end{std::chrono::steady_clock::now()};    
    const std::chrono::duration<double> elapsed_seconds{end - start};

    std::cout << "The ergodic rate is " << ergodic_rate << "nat/Hz/s." << std::endl;
