...
int main(){
    ...
    double den_RIS = 4.0 / (M_PI * (radius_max*radius_max-radius_min*radius_min) / 4.0);
    ...
    auto cluster_cube = CUBE::make_cube_2D<double> (radius_min, radius_max, - M_PI/4.0, M_PI/4.0);
    ...
    for (int i=0; i<=5; i++){
        ...
        auto lt_interference_signal = [&](std::complex<double> s_argument, double distance_arg, double threshold_arg, double x_UE, double y_UE){
            ...
            // auto lt_signal = LaplaceReflectedSignalsCoverageHoleBPP(- 1.0 * s_argument, distance_arg, panelty, cluster_cube, param, 4.0, x_UE, y_UE);
            // auto lt_interference = LT_interference(s_argument, distance_arg, threshold_arg, param, bs_PP, beamformedInterferencePP, scatteredInterferencePP); // Interference from BS + RISs
            auto lt_interference = LT_interference_BSs(s_argument, distance_arg, panelty * threshold_arg, param, bs_PP);  // Interference from BS
            // auto lt_noise = LT_noise(s_argument, noise_power, distance_arg, threshold_arg, param);
            return  lt_interference; // lt_signal *   lt_noise
        };
        ...
        std::cout << "Ergodic rate is " << ergodic_rate << ", with function call " << number_evaluation << " and estimated error " << estimated_error <<   std::endl;
    ...
    std::ofstream out_file( "outputBPP90degree3dB.json" );
